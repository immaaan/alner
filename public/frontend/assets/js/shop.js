(function ($) {
    ...
    $(".minus-cart").on("click", function () {
        ...
        const ini = _currentInput[0]
        const iniId = ini.dataset.id
        ...
        for (let i= 0; i < len; i++) {
            ...
            const puid = inp.dataset.id
            if (puid != iniId) continue
            
            inp.value = ini.value
            inp.dispatchEvent(new Event('input'))
        } 
        
        _currentInput[0].dispatchEvent(new Event('input'))
    });
    $(".plus-cart").on("click", function () {
        ...
        const ini = _currentInput[0]
        const iniId = ini.dataset.id
        const inputX = ini.dataset.x
        const x = this.dataset.target
        _currentInput.val(_number);
        ...
        if (x == inputX) _currentInput[0].dispatchEvent(new Event('input'))
    });
