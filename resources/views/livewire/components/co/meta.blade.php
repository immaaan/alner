<div class="row border mt-100">
    ...
    <div class="col-lg-4 px-0 bg-success">
      <div class="py-4 px-4 height-100" style="background-color: #F1F5F9">
        ...
        <div class="custom-control custom-switch">
          <input type="checkbox" class="custom-control-input" id="customSwitches" @if ($total == 0) disabled @endif>
        ...
        <div class="mt-4">
          {{-- {{var_dump($total)}} --}}
        <button id="submitCheckout" class="btn btn-success width-100  @if (!$totalProduct && !$totaleEmptyBottle) disabled @endif">Checkout</button>
  ...
  $('#customSwitches').on('click', () => {
    console.log($('#customSwitches').is(':checked'))
    if ($('#customSwitches').is(':checked')) {
      let total = {!! json_encode($total) !!}
      let cashback = {!! json_encode($cashbackUser) !!}
      if (total <= cashback) {
        let newCash = cashback - total
        $('#useCashBack').append(`<div class="border-top mt-2 py-3 pt-4 d-flex align-items-center justify-content-between">
              <span class="fs-6 text-success">Sisa <span class="badge bg-success">${formatRupiah(newCash.toString(), 'Rp. ')}</span> akan dimasukkan ke cashback akun kamu</span>
            </div>`)
        window.livewire.emit(`store`, {total: newCash, cashback: cashback})
      }
    }else {
      $('#useCashBack').empty()
    }
  ...
  // console.log({!! json_encode($totalEmptyBottle_TotalProduct) !!})
  // $totaleEmptyBottlePlusShipping = $totaleEmptyBottle+$shipping;
  //           $totalEmptyBottle_TotalProduct = $totaleEmptyBottle-$totalProduct;
